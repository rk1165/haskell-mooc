...
postagePrice wt = if wt <= 500 then 250 else
    if wt <= 5000 then 300 + wt else 6000
...
power n k = powerHelper 1 n k

powerHelper :: Integer -> Integer -> Integer -> Integer
powerHelper acc _ 0 = acc
powerHelper acc base exp = powerHelper (acc * base) base (exp-1)
...
ilog3 num = ilog3Helper 0 num

ilog3Helper :: Integer -> Integer -> Integer
ilog3Helper count 0 = count
ilog3Helper count num = ilog3Helper (count+1) (num `div` 3)
